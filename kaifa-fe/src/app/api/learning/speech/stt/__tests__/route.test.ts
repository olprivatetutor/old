...
function createRequest(audioBlob: Blob | null, token = 'access-token') {
  return {
    ...
    nextUrl: new URL('https://kaifa.test/api/learning/speech/stt?module_id=mod-eng-ix-01-01'),
  } as Parameters<typeof POST>[0];
}

describe('POST /api/learning/speech/stt', () => {
  ...
  it('forwards backend error message', async () => {
    ...
    const response = await POST(createRequest(new Blob(['audio'], { type: 'audio/webm' })));
