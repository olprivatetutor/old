...
describe('POST /api/learning/speech/tts', () => {
  ...
  it('proxies learning speech request and returns audio stream', async () => {
    ...
    const response = await POST(createRequest({ user_text: "Hello, welcome to today's lesson." }));

    expect(response.status).toBe(200);
    expect(response.headers.get('Content-Type')).toBe('audio/mpeg');
    expect(fetchMock).toHaveBeenCalledWith(
      new URL('https://api.example.com/api/learning/speech/tts'),
      {
        method: 'POST',
        cache: 'no-store',
        headers: {
        ...
        body: JSON.stringify({ user_text: "Hello, welcome to today's lesson." }),
      },
    );
  });
